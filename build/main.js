...
app.get("/rubiks/baseMoves", (req, res, next) => res.json({
    Z: [
...
app.get("/rubiks/customMoves", (req, res, next) => res.json([
    {
        key: "Sexy Move",
        action: [
            { slice: "U" },
...
const callback = ({ secure }) => () => console.info(`Server is running on http${secure ? "s" : ""}://${host}:${port}`);
if (nodeEnv === "development") {
    https.createServer({
        pfx: fs.readFileSync("./ssl.pfx"),
        passphrase: "passphrase",
    }, app).listen(port, host, callback({ secure: true }));
}
else {
